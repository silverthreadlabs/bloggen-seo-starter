            ...
            <Tabs.List className='bg-fg-line mx-auto mb-12 flex w-full max-w-md rounded-md p-1 shadow-inner'>
                {tabs.map((tab) => (
                    <Tabs.Trigger
                        key={tab.value}
                        value={tab.value}
                        className='data-[state=active]:bg-bg-base data-[state=active]:text-contrast data-[state=inactive]:text-fg-text data-[state=inactive]:hover:text-fg-text-contrast flex-1 cursor-pointer rounded-sm px-6 py-1.5 text-sm font-medium transition-all duration-200 data-[state=active]:shadow-sm'>
                        {tab.label}
                    </Tabs.Trigger>
                ))}
            </Tabs.List>
