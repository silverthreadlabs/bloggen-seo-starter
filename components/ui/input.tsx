...
const inputVariants = cva(
  'w-full px-4 py-3 rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-primary-solid',
  {
    variants: {
      variant: {
        default: 'bg-bg-bg border border-fg-border text-fg-text hover:border-fg-border-hover focus:border-primary-border',
    ...
    return (
      <div className={cn(containerClassName)}>
        ...
        <input
          ...
          {...props}
        />
        {error && <p className="mt-1 text-sm text-alert-text">{error}</p>}
      </div>
    );
  }
);
